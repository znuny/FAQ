...
sub Data {
    ...
    $Self->{Translation}->{'Core FAQ Data'} = 'FAQコアデータ';
    $Self->{Translation}->{'Dynamic Info'} = '動的情報';
    ...
    $Self->{Translation}->{'Can\'t show history, as no ItemID is given!'} =
        'ItemID が与えられていないため、履歴を表示できませんでした！';
    ...
    $Self->{Translation}->{'Need config option FAQ::Frontend::Overview'} =
        'オプション FAQ::Frontend::Overview の設定が必要です。';
    ...
    $Self->{Translation}->{'Type must be either LastCreate or LastChange or Top10!'} =
        'Type は、 "LastCreate", "LastChange", "Top10" のいずれかでなくてはなりません!';
    ...
    $Self->{Translation}->{'Agent FAQ Related Articles'} = '担当者FAQの関連記事';
    $Self->{Translation}->{'Agent FAQ Related Articles.'} = '担当者FAQの関連記事。';
    $Self->{Translation}->{'Below body'} = '本文の下';
    $Self->{Translation}->{'Below subject'} = '件名の下';
...
注: DefaultColumns には、FAQ属性とダイナミック・フィールド (DynamicField_NameX) のみが設定できます。';
    ...
    $Self->{Translation}->{'Enable counting of articles from FAQ subcategories for FAQ explorer.'} =
        'FAQ一覧でFAQサブカテゴリの記事カウントを有効にする。';
    $Self->{Translation}->{'Enable customer group support permissions.'} = '顧客グループサポート権限を有効にする。';
    $Self->{Translation}->{'Enable customer user permission attributes.'} = '顧客ユーザー権限属性を有効にする。';
    ...
    $Self->{Translation}->{'FAQ AJAX Responder for Richtext.'} = 'リッチテキスト用 FAQ AJAX Responder。';
    ...
    $Self->{Translation}->{'Group to which customer users belong by default (if this setting is enabled).'} =
        '顧客ユーザーがデフォルトで所属するグループ（この設定が有効な場合）。';
    ...
    $Self->{Translation}->{'Output filter to add Java-script to CustomerTicketMessage screen.'} =
        'CustomerTicketMessage画面にJavaScriptを追加する出力フィルタ。';
    $Self->{Translation}->{'Output limit for the related FAQ articles.'} = '関連FAQ記事の出力上限。';
    ...
    $Self->{Translation}->{'Search FAQ Small.'} = 'FAQ検索（コンパクト）。';
    ...
    $Self->{Translation}->{'Show invalid items in the FAQ Explorer result of the agent interface.'} =
        '担当者インターフェイスのFAQ一覧結果に無効な項目を表示する。';
    ...
    $Self->{Translation}->{'The default languages for the related FAQ articles.'} = '関連FAQ記事のデフォルト言語。';
    ...
    $Self->{Translation}->{'Loader module registration for the public interface.'} =
        '公開インターフェイス用ローダーモジュールの登録。';
    $Self->{Translation}->{'Limitation'} = '制限';
